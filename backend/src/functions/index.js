module.exports = {
  validate_user: user => {
    ...
    if(user.DataNascimento != '' && user.DataNascimento != null && typeof user.DataNascimento == "undefined" && user.DataNascimento.search(/^[0-3][0-9]\/[0-1][0-9]\/[0-9][0-9][0-9][0-9]$/g) == -1) err.push({DataNascimento: 'Campo não passou na validação'});
    if(user.CEP != '' && user.CEP != null && typeof user.CEP == "undefined" && user.CEP.search(/^[0-9][0-9][0-9][0-9][0-9]-[0-9][0-9][0-9]$/g) == -1) err.push({CEP: 'Campo não passou na validação'});
